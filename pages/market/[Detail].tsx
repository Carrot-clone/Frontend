  ...
  return itemDetail !== undefined ? (
    <section className='w-full h-full overflow-x-auto scrollbar-hide bg-white rounded-[45px] font-soojin text-[15px]'>
      <div>
        ...
        >
          {itemDetail.images.map((value: any, index: number) => (
            <Fragment key={index}>
              <SwiperSlide>
                <img src={value.image} alt='' className='w-full h-[100%]' />
              </SwiperSlide>
            </Fragment>
          ))}
        </Swiper>
      ...
      <div className='w-full h-full p-2'>
        ...
        <div className='p-2'>
          {itemDetail.category} | {dayjs(itemDetail.createdAt).format('YYYY-MM-DD')}
        </div>
      ...
      {/* <div className='w-full h-16 border bg-black'></div> */}
    </section>
  ) : (
    <div>Loading...</div>
  );
